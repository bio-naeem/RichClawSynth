...
架构（建立在 step4 产出的 workspace bundle 之上）：
  - 每个 workspace 对应 1 个独立 agent
  - workspace 间并发执行（ThreadPoolExecutor），workspace 内串行
  - 保留 LLM 预筛选 + 断点续跑能力

用法：
  python step5_file_generate.py
"""
...
from pipeline_common import load_project_dotenv, normalize_model_name
...
# ============================================================
def parse_args():
    parser = argparse.ArgumentParser(description="Batch generate input files for benchmark queries.")
    parser.add_argument("--hub", type=Path, help="Source workspace hub directory (Step 4 output)")
    parser.add_argument("--base", type=Path, help="Base directory for temporary workspaces")
    parser.add_argument("--results", type=Path, help="Directory to save dialog logs and summary")
    parser.add_argument("--model", default="aliyun-qwen/qwen3.6-plus", help="OpenClaw agent model")
...
MAX_DOMAIN_PARALLEL = 10

# LLM 预筛选模型配置：复用主流水线的 OpenAI/OpenAI-compatible 环境变量
FILTER_API_BASE = os.environ.get("FILTER_API_BASE", os.environ.get("OPENAI_API_BASE", "https://open.bigmodel.cn/api/paas/v4/"))
FILTER_API_KEY = os.environ.get("FILTER_API_KEY", os.environ.get("OPENAI_API_KEY", ""))
FILTER_MODEL = normalize_model_name(os.environ.get("FILTER_MODEL", os.environ.get("OPENAI_MODEL", "glm-5.1")))
FILTER_TIMEOUT = int(os.environ.get("FILTER_TIMEOUT", os.environ.get("OPENAI_TIMEOUT", "50")))
FILTER_WORKERS = int(os.environ.get("FILTER_WORKERS", "1"))
...
# 全局状态（线程安全）
# ============================================================
# 日志写入锁（多线程预筛选阶段并发写同一 log 文件）
_log_lock = threading.Lock()

# workspace 初始化锁：每个 workspace 的临时目录只同步一次
_setup_workspaces: set[str] = set()
_setup_workspaces_lock = threading.Lock()

# CLI 锁：防止高并发增删 Agent 导致 openclaw 系统注册表竞争
...
def needs_input_files(query: str) -> bool | None:
    """调用轻量 LLM 判断 query 是否需要预生成输入文件。
    返回 True=需要, False=不需要, None=判断失败（保守起见当作需要）。
    """
    if not FILTER_API_KEY:
        print("  ⚠️ 预筛选缺少 FILTER_API_KEY / OPENAI_API_KEY，默认当作需要输入文件")
        return None
    url = f"{FILTER_API_BASE.rstrip('/')}/chat/completions"
    headers = {
...
def get_workspace_path(ws_name: str) -> Path:
    """返回某个 workspace 对应的临时工作目录"""
    return WORKSPACE_BASE / f"{ws_name}_workspace"


def get_agent_name(ws_name: str) -> str:
    """返回某个 workspace 对应的 agent 名称（加前缀防冲突）"""
    return f"fg_{ws_name}"


def _agent_exists(agent_name: str) -> bool:
    """通过检查 ~/.openclaw/agents/{agent_name} 目录是否存在来判断 agent 是否已注册"""
    return (Path.home() / ".openclaw" / "agents" / agent_name).exists()


def sync_workspace(ws_dir: Path, tmp_ws: Path):
    """将 hub 中的 workspace 内容同步到临时 workspace，并软链接 claw-input-file-generator skill"""
    ...
    # 只软链接 claw-input-file-generator skill
    if SKILLS_SOURCE.exists() and SKILLS_SOURCE.is_dir():
        target_skills = tmp_ws / "skills"
        target_skills.mkdir(parents=True, exist_ok=True)
        target_skill = target_skills / "claw-input-file-generator"
        try:
            if target_skill.is_symlink() or target_skill.is_file():
                target_skill.unlink()
            elif target_skill.exists():
                shutil.rmtree(target_skill)
            target_skill.symlink_to(SKILLS_SOURCE, target_is_directory=True)
        except Exception as e:
            print(f"  ⚠️ [{ws_dir.name}] 创建 skill 软链接失败 claw-input-file-generator: {e}")


def ensure_workspace(ws_dir: Path):
    """确保临时 workspace 已准备好（拷贝基准文件），只需跑一次即可"""
    ws_name = ws_dir.name
...
def create_agent(ws_name: str):
    """为单个 task 创建一个崭新的 agent"""
    agent_name = get_agent_name(ws_name)
    tmp_ws = get_workspace_path(ws_name)
    cmd = [
        OPENCLAW_BIN, "agents", "add", agent_name,
        "--model", MODEL,
    ...
    with open(log_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                # 只有 success/skip 不重跑；failed/timeout/error/empty_payloads 都重跑
...
def cleanup_single_agent(agent_name: str, silent: bool = False):
    """彻底删除单个 agent，并清除其本地记录"""
    cmd = [OPENCLAW_BIN, "agents", "delete", agent_name, "--force"]
    with _agent_cli_lock:
        result = subprocess.run(cmd, capture_output=True, text=True)
...
def run_prefilter(todo: list[dict], ws_name: str, log_file: Path
                  ) -> tuple[list[dict], int]:
    """并发预筛选，返回 (需要生成文件的记录列表, 跳过数量)"""
    need_generate = []
    skip_count = 0
...
def process_workspace(ws_dir: Path) -> list[dict]:
    ...
    # 即使无需调用 OpenClaw，也保留带后缀的 filegen workspace 副本。
    ...
    for idx, rec in enumerate(need_generate):
        rid = rec["id"]
        query = rec["result"]

        try:
            # 每个 task 前先清理上次残留，并重建纯净 agent 环境
            ...
            # 如果 payloads 为空，标记为 empty_payloads 状态
            ...
            # 每个 task 结束后立即销毁 agent，避免上下文/会话/后台进程残留
            cleanup_single_agent(agent_name, silent=True)
...
def run_all():
    ...
    if summary['this_run_failed'] or summary['this_run_timeout'] or summary['this_run_error']:
        print(f"   💡 重跑脚本会自动续跑 failed/timeout/error 记录")
    print(f"{'=' * 60}")
